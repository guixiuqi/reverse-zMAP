...
#    print (chrom_dict.keys())    
    ...
    plt.scatter(10**7.3,total_length+10**8,c="red",s=50)
    plt.text(10**7.8,total_length+10**8,"Beta>0",va="center",size=20)
...
if __name__=="__main__":
    ...
    parser.add_argument("--mutation_f",help="somatic_mutation_f",required=True)
    parser.add_argument("--z_statistic_matrix",help="z-transformed log2-intensity",required=True)
    parser.add_argument("--covariates_f",help="covariateds",required=True)
    parser.add_argument("--gene_tss_location",help="gene_tss_location",required=True)
    parser.add_argument("--chr_length",help="chr_length",required=True)
    parser.add_argument("--fdr",help="fdr",type=float, default=0.05)
    parser.add_argument("--outdir",help="outdir for result",required=True)
    argv = vars(parser.parse_args())
    mutation_f = argv['mutation_f']
    z_statistic_matrix = argv['z_statistic_matrix']
    covariates_f = argv['covariates_f']
    gene_tss_location = argv['gene_tss_location']
    chr_length = argv['chr_length']
    fdr = float(argv['fdr'])
    outdir = argv['outdir']
    mkdir(outdir)
    ...
    print("Only %s common samples in three input files were included in the analysis."%(str(len(overlap_sample))))
...
#    print('%s Proteins detected in at least half of the samples are included in the mutation-association analysis.')
    ...
    print('%s Proteins detected in at least half of the samples are included in the mutation-association analysis.'%(str(len(z_df))))
