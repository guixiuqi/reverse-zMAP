...
import sys
import re
import reverse_zmap_qc
import reverse_zmap_combine
import reverse_zmap_r2_distribution_of_linear_fitting
import reverse_zmap_r2_distribution_of_nonlinear_fitting
import reverse_zmap_r2_distribution_of_nonlinear_fitting_for_intercept_u
...
def normalization(raw_intensity_df,l=1.5):
...
#    plt.ylim(0,30)
    ...
    plt.subplot(223)
    plt.scatter(log2_intensity_df[ref],
 ...
 #   plt.ylim([min_value-1,max_value+1])
   ...
   # plt.plot([0,30],[0,30])
    ...
    plt.subplot(224)
    plt.scatter(log2_normalized_intensity_df[ref],
...
def MA_linear_regression(log2_normalized_intensity_df,wsize=400,step=100,middle_percent = 0.5):
...
def fit_natural_model(log2_normalized_intensity_df,all_window_mean_value_list,all_window_variance_list,
...
#    plt.scatter(all_window_mean_value_list[0:start]+all_window_mean_value_list[end:],all_window_variance_list[0:start]+all_window_variance_list[end:],facecolors='none', edgecolors="red",label = "Not used for model fitting")
    ...
    plt.plot(a_value[100*start+200:-100*start],nonlinear_model.predict(a_value[100*start+200:-100*start]),"--",c="darkorange",linewidth=3)
    plt.ylim(0,max(nonlinear_model.predict(a_value[100*start+200:-100*start]))+0.5)    
    plt.xlabel("log$_2$-intensity",size=15)
...
##########################fit exp model#################################################
def fit_exp_model(log2_normalized_intensity_df,all_window_mean_value_list,all_window_variance_list,used_for_model_fitting=[0.05,0.95]):
...
#                    facecolors="none",s = 50,edgecolors = "red",label = "Not used for model fitting" )
        ...
        plt.plot(a_value[100*start:-100*start], func_nl_lsq(a_value[100*start:-100*start], *popt), '--',linewidth = 3,color = "darkorange",
             label = "Fitted exponential funcation:\n$\sigma^2=exp(%s-%s*S)+%s$"%(str("%.3f"%(popt[0])),
                                                                   str("%.3f"%(-popt[1])),str("%.3f"%(popt[2]))))
        plt.ylim(0,max(func_nl_lsq(a_value[100*start:-100*start], *popt))+0.5)
    else:
        ...
        plt.scatter(x_model, y_model,s=50,facecolors='none', edgecolors="green",label = "Used for model fitting" )
        plt.plot(a_value[100*start+200:-100*start], func_nl_lsp_no_constant_term(a_value[100*start+200:-100*start], *popt), '--',linewidth = 3,color = "darkorange",
...
#    print(paired_df_list)
    ...
    for paired_df in paired_df_list:
        ...
        log2_normalized_intensity_df,sample,sample_outdir = normalization(new_paired_df) 
        ...
        left = 0.05
        right = 0.95
        start = int(len(all_window_mean_value_list)*left)
        end = int(len(all_window_mean_value_list)*right)
        ...
        if final_method == "natural_cubic_spline":
            nonlinear_model,R_square = fit_natural_model(log2_normalized_intensity_df,all_window_mean_value_list,\
                                all_window_variance_list,used_for_model_fitting=[left,right])
            ...
            if R_square > 0.5:
                ...
                for index in log2_normalized_intensity_df.index:
                    if log2_normalized_intensity_df.loc[index,"A value"]< all_window_mean_value_list[start]:
                        log2_normalized_intensity_df.loc[index,"estimated variance"] = nonlinear_model.predict(np.asarray(all_window_mean_value_list[start:end]))[0]

                    elif log2_normalized_intensity_df.loc[index,"A value"] > all_window_mean_value_list[end]:
                        log2_normalized_intensity_df.loc[index,"estimated variance"] = nonlinear_model.predict(np.asarray(all_window_mean_value_list[start:end]))[-1]
                    else:
                        pass
            else:
                log2_normalized_intensity_df["estimated variance"] = \
            ...
            popt,exp_R_square = fit_exp_model(log2_normalized_intensity_df,all_window_mean_value_list,all_window_variance_list,used_for_model_fitting=[left,right])
            ...
            if exp_R_square > 0.5:
                ...
                for index in log2_normalized_intensity_df.index:
                    ...
                    if log2_normalized_intensity_df.loc[index,"A value"]< all_window_mean_value_list[start]:
                        log2_normalized_intensity_df.loc[index,"estimated variance"] = exp_function(all_window_mean_value_list[start],*popt)
                        
                    elif log2_normalized_intensity_df.loc[index,"A value"] > all_window_mean_value_list[end]:
                        log2_normalized_intensity_df.loc[index,"estimated variance"] = exp_function(all_window_mean_value_list[end],*popt)
