...
def pca_analysis(pearsonr_cor):
    ...
    print('explained variance ratio (top five components): %s'
...
def correlation(df,outdir,method="pearsonr"):
    correlation_array=[]
    for i in df.columns:
        cc_list=[]
        for j in df.columns:
...
def cluster_(correlation_df,color_df,outdir,condition_color_dict):
    patch_list = []
...
if __name__=="__main__":
    ...
    mkdir(outdir)
"""
def sample_quality_control_(z_statistic_matrix,sample_info,outdir):
    ...
    correlation_df = correlation(z_df,outdir,method="pearsonr")
    color_df = get_color_list(correlation_df.index,sample_info_df,batch_color_dict,condition_color_dict)
#    print(color_df)
    print("Performing sample-based hierarchical clustering.")    
    cluster_(correlation_df,color_df,outdir,condition_color_dict)
...
if __name__=="__main__":
    ...
    mkdir(outdir)
    sample_quality_control_(z_statistic_matrix,sample_info,outdir)
