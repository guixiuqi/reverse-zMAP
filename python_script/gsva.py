...
def prepare_gsva_kegg(z_statistic_matrix,sample_info,gmt,outdir,type_="kegg",top_n=50,adj_P_Val=0.05):
    ...
    out.write('group_info <- group_info[common_names, , drop = FALSE] \n')
    ...
    overlap_sample = list(set(z_df.columns)&set(condition_df.index))
    condition_df = condition_df.loc[overlap_sample]    
    ...
    g = sns.clustermap(diff_score_df,col_cluster=False,cmap="bwr",col_colors=sample_color_list,xticklabels=True,yticklabels=True,row_cluster=True,
    ...
    l2.set_title(title='Sample condition',prop={'size':12})
    ...
    plt.savefig("%s"%("%s/%s_gsva_differential_pathway_activity_top_%s_adj.P.Val_%s.png"%(outdir,type_,str(top_n),adj_P_Val)),dpi=300,bbox_inches="tight")
