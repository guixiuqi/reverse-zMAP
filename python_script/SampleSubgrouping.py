...
def prepare_ConsensusClusterPlus(file_,outdir): 
    ...
    out.write("rcc = ConsensusClusterPlus(dc,maxK=8,reps=1000,pItem=0.8,pFeature=1,title='euclidean_km', \
                           distance='euclidean',clusterAlg='km',plot='pdf',seed=1262118322)\n")

    out.write("cluster <- rcc[[2]]$consensusClass\n")
    out.write("write.csv(cluster,file='%s', quote = FALSE)\n"%(outdir+'/cluster_2.csv'))
    ...
    out.write("cluster <- rcc[[6]]$consensusClass\n")
    out.write("write.csv(cluster,file='%s', quote = FALSE)\n"%(outdir+'/cluster_6.csv'))
    out.write("cluster <- rcc[[7]]$consensusClass\n")
    out.write("write.csv(cluster,file='%s', quote = FALSE)\n"%(outdir+'/cluster_7.csv'))
    out.write("cluster <- rcc[[8]]$consensusClass\n")
    out.write("write.csv(cluster,file='%s', quote = FALSE)\n"%(outdir+'/cluster_8.csv'))
...
def sample_subgrouping_(z_statistic_matrix,sample_info,sample_type,outdir,top_n):
...
if __name__=="__main__":
    ...
    mkdir(outdir)
    sample_subgrouping_(z_statistic_matrix,sample_info,sample_type,outdir,top_n)
